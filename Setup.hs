...
--
getLdVersion :: Verbosity -> FilePath -> IO [Int]
getLdVersion verbosity ldPath = do
  ...
  let versionParsed = map read versionParts
  ...
  let handleError :: SomeException -> IO [Int]
      handleError e = do throw $ userError (printf "cannot parse ld version string: `%s`. Parsing exception: `%s`" ldVersionString (show e))
                         return [] -- bogus return -> we throw above

  returnValue <- catch (evaluate versionParsed) handleError
  return returnValue
...
validateLinker verbosity (Platform X86_64 Windows) db = catch validateInner handleError
  where
    handleError :: SomeException -> IO ()
    handleError e = warn verbosity $ "cannot verify linker version: " ++ show e
    validateInner = do
    ...
    postConfHook args flags pkg_descr lbi
      = let verbosity = fromFlag (configVerbosity flags)
        ...
        in do
          ...
          validateLinker verbosity currentPlatform $ configPrograms  flags
