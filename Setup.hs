...
getRealLdPath verbosity programDb =
    ...
    case lookupProgram ghcProgram programDb of
        ...
        Just configuredGhc -> do
            ...
            let presumedLdPath = (takeDirectory . takeDirectory) ghcPath </> "mingw" </> "x86_64-w64-mingw32" </> "bin" </> "ld.exe"
...
validateLinker _ _ _ = return ()
    ...
    -- catch validateInner handleError
--   where
...
--       ldVersion <- (programFindVersion ldProgram) verbosity ldPath
--       case ldVersion of
--           Just version ->
--       when (ldVersion < [2,25,1]) $ die $ linkerBugOnWindowsMsg ldPath
-- validateLinker _ _ _ = return ()
